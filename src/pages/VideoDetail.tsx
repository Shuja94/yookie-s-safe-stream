  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="px-4 md:px-8 pt-4">
        <button onClick={() => navigate(-1)} className="p-2 rounded-lg hover:bg-muted text-muted-foreground transition-colors">
      ...
      >
        <div className="relative rounded-card overflow-hidden cursor-pointer group" onClick={() => navigate(`/player/${video.id}`)}>
      ...
      >
        ...
        <div className="flex gap-3 mb-8">
          <button
            ...
            className="inline-flex items-center gap-2 px-6 py-3 rounded-full gradient-hero text-primary-foreground font-medium hover:shadow-lg transition-all"
          >
            <Play className="w-5 h-5 fill-current" /> Watch Now
          </button>
          <button
            onClick={handleFavorite}
            className={`inline-flex items-center gap-2 px-5 py-3 rounded-full border font-medium transition-all ${
        ...
        {video.tags.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-8">
            {video.tags.map(tag => (
              <span key={tag} className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-muted text-muted-foreground text-xs">
      ...
      {recommendations.length > 0 && (
        <VideoRow title="More for Yookie" delay={0.2}>
